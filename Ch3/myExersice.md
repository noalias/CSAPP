...
01 23 45 67 89 AB CD EF|%rbx中内容
                       |分配的缓冲区
                       |buff = %rsp
2. 填图
...
33 32 31 30 39 38 37 36|%rbx中内容
35 34 33 32 31 30 39 38|分配的缓冲区
37 36 35 34 33 32 31 30|buff = %rsp

3. 返回的地址：0x40034
4. %rbx
5. malloc没有为'\0'分配内存,检查返回值是否为NULL
#### 3.47
1. 0x2000 = 0010 0000 0000 0000 = 2^13
2. 2^13 / 128 = 64
#### 3.48
1. a:buf->%rsp,v->%rsp+24;  
   b:buf->%rsp+16,v->%rsp+8
2. 在有保护的代码中，v更靠近栈顶，所以v将不会被缓冲去溢出破坏
#### 3.49
1. -16 -> 0x1 00 00 00 00 00 00 00 00 - 0x1 00 00 = 0x 11 11 11 11 11 11 00 00 -> ~0x1111 & 8n+22 -> n为奇数时，8n+8; 偶数8n+16; s1减去上述的值就为s2
2. 2.3.7中使用了**偏置**得到商的上限，这里将得到离栈顶最近的、且为8的倍数的地址，即与8对齐的地址
